...
function checkAnswers() {
  ...
  questions.forEach(question => {
    const correctAnswer = question.getAttribute('data-correct-answer').trim().toLowerCase();
    ...
    if (selectedOption) {
      ...
      const userLetter = selectedOption.value.trim().toLowerCase();
      // option text after ") "
      const optionText = selectedOption.parentNode.innerText.split(') ')[1].trim().toLowerCase();

      if (userLetter === correctAnswer || optionText === correctAnswer) {
  ...
  .then(data => console.log('save_score.php response:', data))
